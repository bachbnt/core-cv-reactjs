...
const cvPath = resolve(root, 'src/data/my-cv.json');
const cv = await readJson(cvPath);

if (isDryRun) {
  console.log(
    `Dry run OK: ${cv.candidate?.name ?? 'Unknown candidate'} from ${cvPath}`,
  );
  process.exit(0);
}
...
await firestore.collection('user').doc('cv').set(cv);

console.log(`Uploaded ${cvPath} to Firestore user/cv`);
